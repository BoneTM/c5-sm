...
public void OnPluginStart() {
  ...
  g_teamNameCookie = new Cookie("c5_pug_teamname", "C5 Pug team name", CookieAccess_Protected);
  g_teamFlagCookie = new Cookie("c5_pug_teamflag", "C5 Pug team flag (2-letter country code)", CookieAccess_Protected);
  g_teamLogoCookie = new Cookie("c5_pug_teamlogo", "C5 Pug team logo", CookieAccess_Protected);
  g_CanPlayerCustomCvar = CreateConVar(
      "sm_c5_pug_can_player_custom", "1",
  ...
  BuildPath(Path_SM, configFile, sizeof(configFile), "configs/c5_pug/teamnames.cfg");
...
Menu CreateTeamMenu(int client)
{
  ...
  menu.SetTitle("个人队标设置:\n当你为队长时, 队伍设置将会生效");
  AddMenuOption(menu, "matchteam", "使用战队队标");
  AddMenuOption(menu, "flag", "设置旗帜\n当前旗帜: %s", g_PlayerData[client].flag[0] ? g_PlayerData[client].flag : "无");
  AddMenuOption(menu, "name", "设置队名\n当前队名: %s", g_PlayerData[client].name[0] ? g_PlayerData[client].name : "无");
  AddMenuOption(menu, "name", "设置Logo\n当前Logo: %s", g_PlayerData[client].logo[0] ? g_PlayerData[client].logo : "无");
...
public Action Command_Team(int client, int args) {
  if (g_CanPlayerCustomCvar.BoolValue)
  {
    CreateTeamMenu(client).Display(client, MENU_TIME_FOREVER);
  }
