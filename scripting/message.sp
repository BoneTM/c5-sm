...
{
	g_MessagePrefix = CreateConVar("sm_message_prefix", "[{GREEN}God{NORMAL}]", "message prefix");
  	AutoExecConfig(true, "message", "sourcemod");
}

public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max) {
  CreateNative("Message", Native_Message);
  CreateNative("MessageClean", Native_MessageClean);
  CreateNative("MessageToAll", Native_MessageToAll);

  RegPluginLibrary("message");
...
public int Native_MessageClean(Handle plugin, int numParams) {
  int client = GetNativeCell(1);
  if (client != 0 && (!IsClientConnected(client) || !IsClientInGame(client)))
    return;
