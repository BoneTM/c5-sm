...
public Action CommandWeaponSkins(int client, int args)
{
	if (IsPlayer(client))
	{
		...
		{
			Message(client, "{DARK_RED}%t", "GracePeriod", g_iGracePeriod);
		}
	}
...
void SetWeaponProps(int client, int entity)
{
	...
	if (index > -1)
	{
		...
		weaponData = g_WeaponData[client][team][index];
		if (weaponData.skin == 0) return;
		
		static int IDHigh = 16384;
		SetEntProp(entity, Prop_Send, "m_iItemIDLow", -1);
		SetEntProp(entity, Prop_Send, "m_iItemIDHigh", IDHigh++);

		SetEntProp(entity, Prop_Send, "m_nFallbackPaintKit", weaponData.skin == -1 ? GetRandomSkin(client, index) : weaponData.skin);

		SetEntPropFloat(entity, Prop_Send, "m_flFallbackWear", !g_bEnableFloat || weaponData.floatValue == 0.0 ? 0.000001 : weaponData.floatValue == 1.0 ? 0.999999 : weaponData.floatValue);
		if (g_bEnableSeed && weaponData.seed != -1)
		{
			SetEntProp(entity, Prop_Send, "m_nFallbackSeed", weaponData.seed);
		}
		else
		{
		...
		if(!IsKnife(entity))
		{
			...
			{
				SetEntProp(entity, Prop_Send, "m_nFallbackStatTrak", weaponData.statTrak == 1 ? weaponData.statTrakCount : -1);
				SetEntProp(entity, Prop_Send, "m_iEntityQuality", weaponData.statTrak == 1 ? 9 : 0);
			}
		}
		else
		{
			if(g_bEnableStatTrak)
			{
				SetEntProp(entity, Prop_Send, "m_nFallbackStatTrak", weaponData.statTrak == 0 ? -1 : g_iKnifeStatTrakMode == 0 ? GetTotalKnifeStatTrakCount(client) : weaponData.statTrakCount);
			}
			SetEntProp(entity, Prop_Send, "m_iEntityQuality", 3);
		}
		if (g_bEnableNameTag && strlen(weaponData.nameTag) > 0)
		{
			SetEntDataString(entity, FindSendPropInfo("CBaseAttributableItem", "m_szCustomName"), weaponData.nameTag, 128);
		}
