...
public int OvertimeMenuHandler(Menu menu, MenuAction action, int param1, int selection)
{
	switch(action)
	{
      ...
      if (StrEqual(info, "on"))
      {
        ...
        MessageToAll("%T", "OvertimeVoteOn", LANG_SERVER, client, g_OvertimeMenuCounter, g_PlayersPerTeam * 2);
      }
      else if(StrEqual(info, "off"))
      {
        MessageToAll("%T", "OvertimeVoteOff", LANG_SERVER, client, g_OvertimeMenuCounter, g_PlayersPerTeam * 2);
      }
    }
    case MenuAction_VoteCancel, MenuAction_VoteEnd:
    {
      ...
      {
        MessageToAll("%T", "OvertimeDefault", LANG_SERVER);
      }
      else
      {
        int result = param1;
      ...
      {
        MessageToAll("%T", "OvertimeEndOn", LANG_SERVER);
      }
      else
      {
        MessageToAll("%T", "OvertimeEndOff", LANG_SERVER);
