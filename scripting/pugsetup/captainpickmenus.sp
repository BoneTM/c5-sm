...
public int InitialChoiceHandler(Menu menu, MenuAction action, int param1, int param2) {
  if (action == MenuAction_Select) {
    ...
    if (StrEqual(choice, "player")) {
      MessageToAll("%T", "InitialPickPlayerChoice", client, captString);
      SideMenu(g_capt2);
    } else if (StrEqual(choice, "side")) {
      MessageToAll("%T", "InitialPickSideChoice", client, captString);
      SideMenu(g_capt1);
    } else {
...
public int SideMenuHandler(Menu menu, MenuAction action, int param1, int param2) {
  if (action == MenuAction_Select) {
    ...
    MessageToAll("%t", "SideChoiceSelected", captString, teamString);
  ...
  if (IsPickingFinished()) {
    ...
    if (IsValidClient(client)) {
      ...
      if (AddPlayersToMenu(menu) > 0) {
        ...
        MessageToAll("Not enough players for picking, aborting the game.");
        EndMatch(false);
        delete menu;
      }
    } else {
      MessageToAll("A captain is missing, aborting the game.");
      EndMatch(false);
    }
  }
...
public int PlayerMenuHandler(Menu menu, MenuAction action, int param1, int param2) {
  ...
  if (action == MenuAction_Select) {
    ...
    if (IsPlayer(selected)) {
      ...
      MessageToAll("%t", "PlayerPickChoice", captName, selectedName);
    ...
    MessageToAll("Failed to get captain pick. Aborting the game.");
    EndMatch(false);
