...
ArrayList g_KnifeCvarRestore = null;
...
public Action Timer_AnnounceKnife(Handle timer) {
  ...
  for (int i = 0; i < 5; i++)
    MessageToAll("%t", "KnifeRound");
  return Plugin_Handled;
}


// Add overtime vote by Bone =========start
public int KnifeVoteMenuHandler(Menu menu, MenuAction action, int param1, int selection)
{
	switch(action)
	{
      ...
      if (StrEqual(info, "T"))
      {
        MessageToAll("%T", "ChooseTeamT", LANG_SERVER, client);
        FakeClientCommand(client, "sm_t");
      }
      else if(StrEqual(info, "CT"))
      {
        MessageToAll("%T", "ChooseTeamCT", LANG_SERVER, client);
...
Menu CreateKnifeVoteMenu() {
  Menu menu = new Menu(KnifeVoteMenuHandler);

  menu.SetTitle("%T", "ChooseTeamTitle", LANG_SERVER);
  
  char buffer[60];
  Format(buffer, sizeof(buffer), "%T", "T", LANG_SERVER);
  menu.AddItem("T", buffer);
  Format(buffer, sizeof(buffer), "%T", "CT", LANG_SERVER);
  ...
  menu.DisplayVote(clients, counter, 20);

  return menu;
}

// Add overtime vote by Bone =========end
...
public Action Command_Stay(int client, int args) {
  if (AwaitingDecision(client)) {
    ...
    Message(client, "%t", "KnifeRoundVoteStay");
...
public Action Command_Swap(int client, int args) {
  if (AwaitingDecision(client)) {
    ...
    Message(client, "%t", "KnifeRoundVoteSwap");
    HandleKnifeDecisionVote();
  }
  return Plugin_Handled;
}
