...
public int FriendlyFireMenuHandler(Menu menu, MenuAction action, int param1, int selection)
{
	switch(action)
	{
      ...
      if (StrEqual(info, "on"))
      {
        MessageToAll("%T", "FriendlyFireVoteOn", LANG_SERVER, client);
      }
      else if(StrEqual(info, "off"))
      {
        MessageToAll("%T", "FriendlyFireVoteOff", LANG_SERVER, client);
      }
    }
    case MenuAction_VoteCancel, MenuAction_VoteEnd:
    {
      ...
      {
        MessageToAll("%T", "FriendlyFireRandomly", LANG_SERVER);
      ...
      if (res == 1)
      {
        MessageToAll("%T", "FriendlyFireEndOn", LANG_SERVER);
        g_EnableFriendlyFire = true;
      }
      else
      {
        MessageToAll("%T", "FriendlyFireEndOff", LANG_SERVER);
...
{
  if (!g_EnableFriendlyFire && Pugsetup_IsMatchLive())
  {
