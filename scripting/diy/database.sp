	...
	if(IsPlayer(clientIndex))
	{
		...
		else if (results.RowCount == 0)
		{
			...
			if(GetClientAuthId(clientIndex, AuthId_Steam2, steamid, sizeof(steamid), true))
			{
				...
				{
					g_WeaponData[clientIndex][CS_TEAM_T][i].SetDefault();
					g_WeaponData[clientIndex][CS_TEAM_CT][i].SetDefault();
				}
		...
		else
		{
			if(results.FetchRow())
			{
				...
				{
					g_WeaponData[clientIndex][CS_TEAM_T][j].skin = results.FetchInt(i);
					g_WeaponData[clientIndex][CS_TEAM_T][j].floatValue = results.FetchFloat(i + 1);
					g_WeaponData[clientIndex][CS_TEAM_T][j].statTrak = results.FetchInt(i + 2);
					g_WeaponData[clientIndex][CS_TEAM_T][j].statTrakCount = results.FetchInt(i + 3);
					results.FetchString(i + 4, g_WeaponData[clientIndex][CS_TEAM_T][j].nameTag, 128);
					g_WeaponData[clientIndex][CS_TEAM_T][j].seed = results.FetchInt(i + 5);

					g_WeaponData[clientIndex][CS_TEAM_CT][j].skin = results.FetchInt(i + 6 * 54);
					g_WeaponData[clientIndex][CS_TEAM_CT][j].floatValue = results.FetchFloat(i + 1 + 6 * 54);
					g_WeaponData[clientIndex][CS_TEAM_CT][j].statTrak = results.FetchInt(i + 2 + 6 * 54);
					g_WeaponData[clientIndex][CS_TEAM_CT][j].statTrakCount = results.FetchInt(i + 3 + 6 * 54);
					results.FetchString(i + 4 + 6 * 54, g_WeaponData[clientIndex][CS_TEAM_CT][j].nameTag, 128);
					g_WeaponData[clientIndex][CS_TEAM_CT][j].seed = results.FetchInt(i + 5 + 6 * 54);
				}
...
void AddWeaponColumns(bool mysql, const char[] weapon, bool seedColumn = true)
{
	...
	txn.AddQuery(query);
	Format(query, sizeof(query), "ALTER TABLE %sweapons ADD %s_tag varchar(128) NOT NULL DEFAULT ''", g_TablePrefix, weapon);
	txn.AddQuery(query);
	if (seedColumn)
	{
