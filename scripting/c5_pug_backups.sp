...
#define TEMP_MATCHCONFIG_BACKUP_PATTERN "c5_pug_match_config_backup%d.txt"
#define TEMP_VALVE_BACKUP_PATTERN "c5_pug_temp_backup%d.txt"
...
public void OnPluginStart() {
  ...
  RegAdminCmd("sm_loadbackup", Command_LoadBackup, ADMFLAG_CHANGEMAP, "Loads a match backup");
  RegAdminCmd("sm_listbackups", Command_ListBackups, ADMFLAG_CHANGEMAP, "Lists match backups for the current matchid or a given one");
}
...
public Action Command_LoadBackup(int client, int args) {
  ...
  if (args >= 1 && GetCmdArg(1, path, sizeof(path))) {
    if (RestoreFromBackup(path)) {
      C5_MessageToAll("BackupLoadedInfoMessage", path);
    } else {
      ReplyToCommand(client, "Failed to load backup %s - check error logs", path);
    }
  } else {
    ReplyToCommand(client, "Usage: .loadbackup <file>");
  }
