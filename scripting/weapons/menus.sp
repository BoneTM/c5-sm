	...
	switch(action)
	{
		case MenuAction_Select:
		{
			if(IsClientInGame(client))
			{
				...
				g_WeaponData[client][team][index].skin = skinId;
		...
		case MenuAction_Select:
		{
			if(IsClientInGame(client))
			{
				...
				else if(StrEqual(buffer, "stattrak"))
				{
					...
					g_WeaponData[client][team][g_iIndex[client]].statTrak = 1 - g_WeaponData[client][team][g_iIndex[client]].statTrak;

					char teamName[4];
					teamName = team == CS_TEAM_T ? "" : "ct_";
					Format(updateFields, sizeof(updateFields), "%s%s_trak = %d", teamName, weaponName, g_WeaponData[client][team][g_iIndex[client]].statTrak);
					UpdatePlayerData(client, updateFields);
				...
				else if (StrEqual(buffer, "applyother"))
				{
					...
					WeaponData otherTeamWeaponData;
					otherTeamWeaponData = g_WeaponData[client][otherTeam][g_iIndex[client]];

					g_WeaponData[client][team][g_iIndex[client]].skin = otherTeamWeaponData.skin;
					g_WeaponData[client][team][g_iIndex[client]].floatValue = otherTeamWeaponData.floatValue;
					g_WeaponData[client][team][g_iIndex[client]].statTrak = otherTeamWeaponData.statTrak;
					g_WeaponData[client][team][g_iIndex[client]].statTrakCount = otherTeamWeaponData.statTrakCount;
					g_WeaponData[client][team][g_iIndex[client]].nameTag = otherTeamWeaponData.nameTag;
					g_WeaponData[client][team][g_iIndex[client]].seed = otherTeamWeaponData.seed;
					...
					Format(updateFields, sizeof(updateFields), "%s%s = %d,			\
																...
																%s%s_tag = '%s',		\
																%s%s_seed = %d",
																otherTeamName, weaponName, otherTeamWeaponData.skin, 
																otherTeamName, weaponName, otherTeamWeaponData.floatValue, 
																otherTeamName, weaponName, otherTeamWeaponData.statTrak, 
																otherTeamName, weaponName, otherTeamWeaponData.statTrakCount, 
																otherTeamName, weaponName, otherTeamWeaponData.nameTag, 
																otherTeamName, weaponName, otherTeamWeaponData.seed);
					UpdatePlayerData(client, updateFields);
...
Menu CreateFloatMenu(int client)
{
	...
	float fValue = g_WeaponData[client][team][g_iIndex[client]].floatValue;
	fValue = fValue * 100.0;
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			if(IsClientInGame(client))
			{
				...
				{
					g_WeaponData[client][team][g_iIndex[client]].floatValue = g_WeaponData[client][team][g_iIndex[client]].floatValue - g_fFloatIncrementSize;
					if(g_WeaponData[client][team][g_iIndex[client]].floatValue < 0.0)
					{
						g_WeaponData[client][team][g_iIndex[client]].floatValue = 0.0;
					}
				...
				else if(StrEqual(buffer, "decrease"))
				{
					...
					g_WeaponData[client][team][g_iIndex[client]].floatValue = g_WeaponData[client][team][g_iIndex[client]].floatValue + g_fFloatIncrementSize;
					if(g_WeaponData[client][team][g_iIndex[client]].floatValue > 1.0)
					{
						g_WeaponData[client][team][g_iIndex[client]].floatValue = 1.0;
					}
					if(g_FloatTimer[client] != INVALID_HANDLE)
					{
						KillTimer(g_FloatTimer[client]);
...
public Action FloatTimer(Handle timer, DataPack pack)
{
	...
	if(IsPlayer(clientIndex))
	{
		...
		char teamName[4];
		teamName = team == CS_TEAM_T ? "" : "ct_";
		Format(updateFields, sizeof(updateFields), "%s%s_float = %.2f", teamName, weaponName, g_WeaponData[clientIndex][team][g_iIndex[clientIndex]].floatValue);
...
Menu CreateSeedMenu(int client)
{
	...
	char buffer[128];
	if (g_WeaponData[client][team][g_iIndex[client]].seed != -1)
	{
		Format(buffer, sizeof(buffer), "%T", "SeedTitle", client, g_WeaponData[client][team][g_iIndex[client]].seed);
	}
	...
	menu.AddItem("seedr", buffer, g_WeaponData[client][team][g_iIndex[client]].seed == -1 ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			if(IsClientInGame(client))
			{
				...
				{
					g_WeaponData[client][team][g_iIndex[client]].seed = -1;
					RefreshWeapon(client, g_iIndex[client]);
				...
				else if (StrEqual(buffer, "cseed"))
				{
					...
					Message(client, "{GREEN}%t", "SeedInstruction");
				}
				else if (StrEqual(buffer, "sseed"))
				{
					if(g_iSeedRandom[client][g_iIndex[client]] > 0) 
					{
						g_WeaponData[client][team][g_iIndex[client]].seed = g_iSeedRandom[client][g_iIndex[client]];
					}
					...
					char teamName[4];
					teamName = team == CS_TEAM_T ? "" : "ct_";
					Format(updateFields, sizeof(updateFields), "%s%s_seed = %d", teamName, weaponName, g_WeaponData[client][team][g_iIndex[client]].seed);
					UpdatePlayerData(client, updateFields);
					CreateTimer(0.1, SeedMenuTimer, GetClientUserId(client));

					Message(client, "{GREEN}%t", "SeedSaved");
				}
				else if (StrEqual(buffer, "seedr"))
				{
					...
					Message(client, "{GREEN}%t", "SeedReset");
				}
			}
		}
		case MenuAction_Cancel:
...
Menu CreateNameTagMenu(int client)
{
	...
	char buffer[128];
	int team = IsWeaponIndexInOnlyOneTeam(g_iIndex[client]) ? CS_TEAM_T : GetClientTeam(client);
	StripHtml(g_WeaponData[client][team][g_iIndex[client]].nameTag, buffer, sizeof(buffer));
	...
	menu.AddItem("delete", buffer, strlen(g_WeaponData[client][team][g_iIndex[client]].nameTag) > 0 ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		...
		case MenuAction_Select:
		{
			if(IsClientInGame(client))
			{
				...
				if(StrEqual(buffer, "nametag"))
				{
					...
					Message(client, "{GREEN}%t", "NameTagInstruction");
				}
				else if(StrEqual(buffer, "delete"))
				{
					...
					int team = IsWeaponIndexInOnlyOneTeam(g_iIndex[client]) ? CS_TEAM_T : GetClientTeam(client);
					g_WeaponData[client][team][g_iIndex[client]].nameTag = "";
					char teamName[4];
					teamName = team == CS_TEAM_T ? "" : "ct_";
...
Menu CreateWeaponMenu(int client)
{
	...
	int team = IsWeaponIndexInOnlyOneTeam(index) ? CS_TEAM_T : GetClientTeam(client);
	bool weaponHasSkin = (g_WeaponData[client][team][index].skin != 0);
	...
	if (g_bEnableFloat)
	{
		float fValue = g_WeaponData[client][team][index].floatValue;
		fValue = fValue * 100.0;
		int wear = 100 - RoundFloat(fValue);
	...
	{
		if (g_WeaponData[client][team][index].statTrak == 1)
		{
