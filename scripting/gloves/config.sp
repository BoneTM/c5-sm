...
public void ReadConfig()
{
	...
	if(g_CustomGloveModel != null) delete g_CustomGloveModel;
	g_CustomGloveModel = new StringMap();
	...
	for (int i = 0; i < langCount; i++)
	{
		...
		// load custom glove
		BuildPath(Path_SM, configPath, sizeof(configPath), "configs/gloves/custom.cfg");
		...
		do {
			...
			do {
				...
				g_CustomGloveModel.SetString(index, model);
				Format(buffer, sizeof(buffer), "%s;%s", group, index);
				menuGloves[langCounter][counter].AddItem(buffer, name);
			} while (KvGotoNextKey(kv));
			KvGoBack(kv);
